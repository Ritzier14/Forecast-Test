...
public sealed class UnmatchedImportWindow : Window
{
    public UnmatchedImportWindow(IEnumerable<UnmatchedImportCombination> items)
    {
        ...
        {
            Header = "Recorded",
            Binding = new Binding(nameof(UnmatchedImportCombination.RecordedAt))
            {
                Converter = new DateTimeDisplayConverter()
            },
            SortMemberPath = nameof(UnmatchedImportCombination.RecordedAt),
            Width = 130
        });
